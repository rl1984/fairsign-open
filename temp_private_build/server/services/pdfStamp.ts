...
export async function appendAuditTrailPage(
  ...
  if (options.sha256) {
    yPosition -= 5;
    drawText(`Document Hash (SHA-256):`, margin, 10, helveticaFont, rgb(0.3, 0.3, 0.3));
    yPosition -= lineHeight;
  ...
  for (const event of sortedEvents) {
    ...
    if (event.metaJson && typeof event.metaJson === "object") {
      ...
      // Signer info
      if (meta.signerEmail) {
        drawText(`Signer: ${meta.signerEmail}${meta.signerRole ? ` (${meta.signerRole})` : ""}`, margin + 10, 9, helveticaFont, rgb(0.4, 0.4, 0.4));
        yPosition -= lineHeight - 2;
      }
